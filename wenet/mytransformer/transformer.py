...
from wenet.mytransformer.attention import PositionalEncoding, RelPositionalEncoding, PositionwiseFeedForward, MultiLayeredConv1d, Conv1dLinear, MultiHeadedAttentionPure
from wenet.mytransformer.utils import *
from wenet.mytransformer.myutils import strtobool, IGNORE_ID
...
class MultiSequential(torch.nn.Sequential):
    ...
    # @torch.jit.export
    # def forward_streaming(self, x, prev_state):
    #     # type: (Tensor, Dict[str, Tensor]) -> Tensor
    #     """Repeat."""
    #     index = 0
    #     for m in self:
    #         x = m.forward_streaming(x, prev_state, index)
    #         index = index + 1
    #     return x
...
class Transformer(torch.nn.Module):
    ...
    def __init__(self, args, input_dim=None, output_dim=None, attention_dim=None, attention_heads=None, linear_units=None, 
        ...
        self.encoders = repeat(
            ...
            lambda lnum: TransformerLayer(
                self.attention_dim,
                MultiHeadedAttentionPure(self.attention_heads, self.attention_dim, self.attention_dropout_rate, self.chunk_size, self.left_chunks, self.pos_enc_class),
    ...
    def forward(self, xs, ilens=None, masks=None):
        ...
        if masks is None:
            ...
            if _chunk_size == -1:
                masks = casual_chunk_mask(ilens, _chunk_size, self.left_chunks).to(xs.device)
            else:
                if not self.training:
                    masks = casual_chunk_mask(ilens, _chunk_size, self.left_chunks).to(xs.device)
                else:
                    masks = ~make_pad_mask(torch.tensor(ilens).to(xs.device)).unsqueeze(1) 
                    masks = add_optional_chunk_mask(xs, masks,
                                                    True,
                                                    True,
                                                    0,
                                                    -1,
