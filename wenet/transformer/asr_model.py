...
from wenet.transformer.decoder import (TransformerDecoder,
                                       BiTransformerDecoder,TransformerDecoderContext)
...
class ASRModelContext(torch.nn.Module):
    """CTC-attention hybrid Encoder-Decoder model"""
    ...
    def __init__(
        ...
        self.use_decoder_context = decoder.use_context
    def forward(
        self,
        speech: torch.Tensor,
        speech_lengths: torch.Tensor,
        ...
        # 1. Encoder
        encoder_out, encoder_mask = self.encoder(speech, speech_lengths)
        bias_hidden = self.context_bias.forword_common(context_list, context_lengths, encoder_out)
        if self.context_bias.if_bias_encoder:
            encoder_out = self.context_bias.forward_encoder(bias_hidden, encoder_out)
        # encoder_out = self.context_bias(context_list, context_lengths, encoder_out)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        # 2a. Attention-decoder branch
        if self.ctc_weight != 1.0:
            loss_att, acc_att = self._calc_att_loss(encoder_out, encoder_mask,
                                                    text, text_lengths,bias_hidden)
        else:
            loss_att = None
    ...
    def attention_rescoring(
        ...
        # encoder_out: (1, maxlen, encoder_dim), len(hyps) = beam_size
        hyps, encoder_out = self._ctc_prefix_beam_search(
            speech, speech_lengths, context_list, context_lengths, beam_size, decoding_chunk_size,
            num_decoding_left_chunks, simulate_streaming)
...
def init_asr_model(configs):
    ...
    elif decoder_type == 'transformercontext':
        decoder = TransformerDecoderContext(vocab_size, encoder.output_size(),
                                    **configs['decoder_conf'])
    else:
    ...
    if context_type != 'nobias':
        context_bias = ContextBias(input_size=encoder.output_size(
        ), output_size=encoder.output_size(), vocab_size=vocab_size, **configs['context_conf'])
